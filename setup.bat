...
cls
echo =================================================================
echo    Nomis BOT SETUP AND RUN SCRIPT by @MeoMunDep
echo =================================================================
echo.
...
if exist "..\node_modules" (
    ...
    npm install user-agents axios meo-forkcy-colors meo-frokcy-utils https-proxy-agent socks-proxy-agent 
    cd %~dp0
) else (
...
if not exist configs.json (
    ...
    echo   "isSkipInvalidProxy": true,>> configs.json
    echo   "referralCode": "YQe1B1-3hK">> configs.json
    echo }>> configs.json
